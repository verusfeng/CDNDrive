...
{'time': 1613238715,
 ...
 'url': 'bdex://17e543b86c128e7a68f477ccf2649ed7d9e81c93'}
    ...
    if os.getenv("COMPUTERNAME") == "LAPTOP-IJTKOVMF":  # my acer laptop..
        ...
        db_url = [each for each in os.getenv("database").split(
            ";") if "sqlite3.db" in each][0].strip()
        logging.info(db_url)
        with sqlite3.connect(db_url) as db:
            ...
            string = replace_sql.format(sha1=sha1, upload_time=upload_time, filename=filename,
                                        size=size, url=url, block_list_base64=block_list_base64)
            db.execute(string)
            db.commit()
        # db.close()
